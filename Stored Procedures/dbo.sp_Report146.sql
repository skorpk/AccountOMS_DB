...
CREATE PROCEDURE [dbo].[sp_Report146]
  @ReportYear int,
  @ReportMonth int,
  @DateRegBegY date,
  @DateRegBegM date,
...
BEGIN
  ...
  select f.DateRegistration, c.id, a.rf_idSMO, c.rf_idMO, m.MUGroupCode, m.MUUnGroupCode, m.MUCode, m.MUSurgery, 
	...
	case when (m.MU like '60.3.%' and not exists (select m2.id from t_Meduslugi m2 where m2.rf_idCase = m.rf_idCase and m2.MU not like '60.3.%')) then null else ps.ENP end as ENP,
