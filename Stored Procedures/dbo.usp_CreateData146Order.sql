...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_CreateData146Order]
		@reportMonth TINYINT,
		@reportYear SMALLINT
as        
...
FROM (
	...
	FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
				...
				m.MES=csg.code       					
	WHERE f.DateRegistration>=@dateStart AND f.DateRegistration<@dateEnd AND c.rf_idV006=1 AND csg.code LIKE '1___9__'
...
FROM (
	...
	GROUP BY c.id,f.CodeM,a.rf_idSMO,p.IDPeople,c.AmountPayment
	UNION ALL
	SELECT c.id,f.CodeM,a.rf_idSMO,7 AS UnitAccounting,0,null,c.AmountPayment,0,null,0 
	FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
				f.id=a.rf_idFiles
				AND a.ReportMonth<=@reportMonth
				AND a.ReportYear=@reportYear
				...
				c.id=m.rf_idCase				
						INNER JOIN RegisterCases.dbo.vw_sprCSG	csg ON
				m.MES=csg.code                       					
	WHERE f.DateRegistration>=@dateStart AND f.DateRegistration<@dateEnd AND c.rf_idV006=2 AND csg.code LIKE '2___9__'
	GROUP BY c.id,f.CodeM,a.rf_idSMO,c.AmountPayment
	-------------------------------------Year------------------------------------------
	UNION ALL
...
GO
GRANT EXECUTE ON  [dbo].[usp_CreateData146Order] TO [db_AccountOMS]
GO
