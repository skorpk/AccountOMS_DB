...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_GetDataForActControl]				
				@codeM CHAR(6)
as
...
----------tag AKT
SELECT @numAct=ISNULL(MAX(NumberAct),0) FROM t_RefActOfSettledAccountBySMO WHERE CodeM=@codeM AND YEAR(DateAct)=@yyyy
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<=@dtEnd AND a.rf_idSMO='34006' AND f.CodeM=@codeM AND a.Letter<>'E'
...
SELECT a.id,t.CODEA,a.idRecord AS CODE,a.rf_idMO AS CODE_MO,a.ReportYear AS [YEAR],a.ReportMonth AS [MONTH],a.Account AS NSCHET,a.DateRegister AS DSCHET,
		...
		,SUM(CASE WHEN c.DateEnd>'20170103' THEN 0 ELSE c.AmountPayment END) AS SUMMAP
		,SUM(CASE WHEN c.DateEnd>'20170103' THEN c.AmountPayment ELSE 0 END) AS SANK_MEK
...
SELECT a.id, c.idRecordCase AS IDCASE,c.GUID_Case AS ID_C,c.rf_idMO AS LPU
	,CASE WHEN c.DateEnd>'20170103' THEN 2 ELSE 1 END AS OPLATA
	,CASE WHEN c.DateEnd>'20170103' THEN 0 ELSE c.AmountPayment END AS SUMP
	,CASE WHEN c.DateEnd>'20170103' THEN c.AmountPayment ELSE 0 END AS SANK_MEK
	,0 AS SANK_MEE,0 AS SANK_EKMP
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
				  INNER JOIN #t t ON
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			a.id=r.rf_idRegistersAccounts
					INNER JOIN dbo.t_Case c ON
			r.id=c.rf_idRecordCasePatient                  
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<=@dtEnd AND a.rf_idSMO='34006' AND f.CodeM=@codeM AND c.DateEnd>'20170103'
		AND NOT EXISTS(SELECT * FROM dbo.t_RefActOfSettledAccountBySMO WHERE rf_idAccounts=a.id)
UNION ALL
SELECT a.id, c.idRecordCase AS IDCASE,c.GUID_Case AS ID_C,e.Reason AS REFREASON
FROM #t t INNER JOIN dbo.t_Act_Accounts_MEEAndEKMP e ON
			t.id=e.id
				INNER JOIN dbo.t_RegistersAccounts a ON
			e.rf_idAccount=a.id          
...
--BEGIN
	...
	select @id, a.rf_idSMO, e.CodeM, e.rf_idAccount, e.rf_idCase, a.id, t.CODEA
	FROM #t t INNER JOIN dbo.t_Act_Accounts_MEEAndEKMP e ON
			t.id=e.id  
			  INNER JOIN dbo.t_RegistersAccounts a ON
			e.rf_idAccount=a.id 
...
SELECT @id
GO
GRANT EXECUTE ON  [dbo].[usp_GetDataForActControl] TO [db_AccountOMS]
GO
