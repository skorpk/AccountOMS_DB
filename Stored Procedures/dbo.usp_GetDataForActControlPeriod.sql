...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_GetDataForActControlPeriod]				
				@codeM CHAR(6),
				@yyyy  SMALLINT,
				@month TINYINT
as
DECLARE @dtEnd DATETIME=GETDATE(),
		...
		@mm CHAR(2)=RIGHT('0'+CAST(@month AS varchar(2)),2),
		@packNum CHAR(3),
		@codeA INT,
		@numAct INT
...
SELECT @numAct=MAX(t.NumAct)
FROM (
		SELECT ISNULL(MAX(NumberAct),0) AS NumAct FROM t_RefActOfSettledAccountBySMO WHERE CodeM=@codeM AND ReportYear=@yyyy
		UNION ALL
		SELECT ISNULL(MAX(CodeA),0) FROM dbo.t_RefActOfSettledAccount_EKMP_MEE WHERE CodeM=@codeM AND ReportYear=@yyyy
	)  t
...
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<=@dtEnd AND a.rf_idSMO='34006' AND f.CodeM=@codeM 
		...
		AND YEAR(GETDATE())=@yyyy AND MONTH(GETDATE())=@month -- AND a.Letter<>'E'
...
;WITH cte
AS(
SELECT DISTINCT a.id,a.NumAct,a.DateAct,a.TypeCheckup,a.OrderCheckup, CASE WHEN a.TypeEx<>'МЭК' THEN 1 ELSE 0 END AS SKONT
FROM dbo.t_Act_Accounts_MEEAndEKMP a
WHERE a.CodeM=@codeM AND YEAR(a.DateAct)=@yyyy AND MONTH(a.DateAct)=@month 
	AND NOT EXISTS(SELECT * FROM dbo.t_RefActOfSettledAccount_EKMP_MEE WHERE rf_idAct_Accounts_MEEAndEKMP=a.id AND rf_idCase=a.rf_idCase AND rf_idAct_Accounts_MEEAndEKMP=a.id)--добавить проверку что бы повторно не выгружались
)
INSERT #t( CodeA ,NAKT ,DAKT ,Kont ,Typek ,SKONT ,TypeAct,id)
SELECT @maxCodeA+ROW_NUMBER() OVER(ORDER BY cte.id),NumAct ,DateAct ,TypeCheckup ,OrderCheckup,SKONT,2,cte.id
FROM cte
------------------------Данные для тега AKT----------------------
...
SELECT a.id,t.CODEA,a.idRecord AS CODE,a.rf_idMO AS CODE_MO,a.ReportYear AS [YEAR],a.ReportMonth AS [MONTH],a.Account AS NSCHET,a.DateRegister AS DSCHET,
		'34006',a.AmountPayment AS SUMMAV
		,SUM(CASE WHEN a.Letter!='E' and c.DateEnd>'20170103' THEN 0 ELSE c.AmountPayment END) AS SUMMAP
		,SUM(CASE WHEN a.Letter!='E' AND c.DateEnd>'20170103' THEN c.AmountPayment ELSE 0 END) AS SANK_MEK
		,0 AS SANK_MEE,0 AS SANK_EKMP
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
				  INNER JOIN #t t ON
			a.id=t.id     
...
SELECT a.id, c.idRecordCase AS IDCASE,c.GUID_Case AS ID_C,c.rf_idMO AS LPU
	,CASE WHEN a.Letter!='E' AND c.DateEnd>'20170103' THEN 2 ELSE 1 END AS OPLATA
	,CASE WHEN a.Letter!='E' AND c.DateEnd>'20170103' THEN 0 ELSE c.AmountPayment END AS SUMP
	,CASE WHEN a.Letter!='E' AND c.DateEnd>'20170103' THEN c.AmountPayment ELSE 0 END AS SANK_MEK
	,0 AS SANK_MEE,0 AS SANK_EKMP
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
				  INNER JOIN #t t ON
...
UNION ALL
SELECT a.id, c.idRecordCase AS IDCASE,c.GUID_Case AS ID_C,c.rf_idMO AS LPU,
		 CASE WHEN c.AmountPayment!=0 AND c.AmountPayment-e.Deduction=0 THEN 2 
		 ...
		 CASE WHEN e.TypeCheckup=1 THEN e.Deduction ELSE 0.0 END AS SANK_MEK,
		 CASE WHEN e.TypeCheckup=2 THEN e.Deduction ELSE 0.0 END AS SANK_MEE,
		 CASE WHEN e.TypeCheckup=3 THEN e.Deduction ELSE 0.0 END AS SANK_EKMP
FROM #t t INNER JOIN dbo.t_Act_Accounts_MEEAndEKMP e ON
			t.id=e.id
				INNER JOIN dbo.t_RegistersAccounts a ON
			e.rf_idAccount=a.id          
...
SELECT a.id, c.idRecordCase AS IDCASE,c.GUID_Case AS ID_C
		,'50' AS REFREASON
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
				  INNER JOIN #t t ON
			...
			a.id=r.rf_idRegistersAccounts
					INNER JOIN dbo.t_Case c ON
			r.id=c.rf_idRecordCasePatient                  
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<=@dtEnd AND a.rf_idSMO='34006' AND f.CodeM=@codeM AND a.Letter!='E' and c.DateEnd>'20170103'
		AND NOT EXISTS(SELECT * FROM dbo.t_RefActOfSettledAccountBySMO WHERE rf_idAccounts=a.id)
UNION ALL
SELECT a.id, c.idRecordCase AS IDCASE,c.GUID_Case AS ID_C,e.CodeReason AS REFREASON
FROM #t t INNER JOIN dbo.t_Act_Accounts_MEEAndEKMP e ON
			t.id=e.id
				INNER JOIN dbo.t_RegistersAccounts a ON
			e.rf_idAccount=a.id          
			...
			r.id=c.rf_idRecordCasePatient                  
			AND c.id=e.rf_idCase
WHERE e.CodeM=@codeM AND t.TypeAct=2 AND e.Reason IS NOT null
...
BEGIN
	...
	INSERT dbo.t_RefActOfSettledAccount_EKMP_MEE( rf_idActFileBySMO ,CodeSMO ,CodeM ,rf_idAccounts ,rf_idCase ,rf_idAct_Accounts_MEEAndEKMP ,CodeA,ReportYear)
	select @id, a.rf_idSMO, e.CodeM, e.rf_idAccount, e.rf_idCase, e.id, t.CODEA, YEAR(e.DateAct) AS ReportYear
	FROM #t t INNER JOIN dbo.t_Act_Accounts_MEEAndEKMP e ON
			t.id=e.id  
			  INNER JOIN dbo.t_RegistersAccounts a ON
			e.rf_idAccount=a.id 
...
SELECT @id
GO
GRANT EXECUTE ON  [dbo].[usp_GetDataForActControlPeriod] TO [db_AccountOMS]
GO
