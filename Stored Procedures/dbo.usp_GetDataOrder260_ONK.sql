...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_GetDataOrder260_ONK]			
AS
DECLARE @reportYear SMALLINT,
		@reportMonth TINYINT

SELECT @reportYear=[YEAR], @reportMonth=[MONTH] FROM dbo.t_260order_ONK WHERE IsUnload IS null        
...
SELECT 
	...
	(
		SELECT CODE ,CODE_MO ,YEAR ,/*t.ReportMonth*/ t.MONTH as [MONTH] ,NSCHET ,DSCHET ,PLAT ,SUMMAV,SUMMAV AS SUMMAP
		,(SELECT N_ZAP ,
				PR_NOV ,(SELECT  ID_PAC ,VPOLIS,SPOLIS ,RTRIM(NPOLIS) 'NPOLIS', ST_OKATO  ,SMO ,RTRIM(NOVOR) 'NOVOR' FROM dbo.vw_Pacient260_ONK 
							WHERE idFile=z.idFile AND rf_idRecordCasePatient=z.rf_idRecordCasePatient AND [YEAR]=@reportYear AND [MONTH]=@reportMonth  FOR XML PATH ('PACIENT'), TYPE)
								...
								,(
								  ...
								  (
									...
									(
									 ...
									 ,PPTR, CASE WHEN LUCH_TIP=3 THEN 2 ELSE ou.LUCH_TIP END AS LUCH_TIP --11.11.2019
		  ...
		  WHERE idFile=t.id AND z.[YEAR]=@reportYear AND z.[MONTH]=@reportMonth FOR XML PATH ('ZAP'), TYPE)
...
UPDATE dbo.t_260order_ONK SET IsUnload=1 WHERE [YEAR]=@reportYear AND [MONTH]=@reportMonth
END
GO
