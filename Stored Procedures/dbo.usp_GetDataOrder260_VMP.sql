...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_GetDataOrder260_VMP]
		
as
DECLARE 	@reportYear SMALLINT,
			@reportMonth TINYINT
SELECT @reportYear=[YEAR], @reportMonth=[MONTH] FROM dbo.t_260order_VMP WHERE IsUnload IS null        
...
SELECT 
	...
	(
		...
		,(SELECT N_ZAP ,
				PR_NOV ,(SELECT  ID_PAC ,VPOLIS,SPOLIS ,RTRIM(NPOLIS) 'NPOLIS',ST_OKATO  ,SMO ,RTRIM(NOVOR) 'NOVOR' FROM dbo.vw_Pacient260_VMP WHERE idFile=z.idFile AND rf_idRecordCasePatient=z.rf_idRecordCasePatient  FOR XML PATH ('PACIENT'), TYPE)
				,(SELECT IDCASE ,USL_OK ,VIDPOM ,FOR_POM ,NPR_MO ,NPR_DATE ,LPU ,DATE_Z_1 ,DATE_Z_2 ,KD_Z ,RSLT ,ISHOD 
					...
					) ,IDSP ,SUMV,OPLATA,SUMP
					/*вставить блок SNK и SANK_IT*/
					,SANK_IT
...
UPDATE dbo.t_260order_VMP SET IsUnload=1 WHERE [YEAR]=@reportYear AND [MONTH]=@reportMonth
END
GO
