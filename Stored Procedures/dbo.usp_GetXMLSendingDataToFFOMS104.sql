...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_GetXMLSendingDataToFFOMS104]
		@reportMonth TINYINT,
		@reportYear SMALLINT,
		@nameFile varchar(20)
AS
...
*/
--для случаев оказания с июня месяца
SELECT t.colXML from(
SELECT (
...
(SELECT ROW_NUMBER() OVER(ORDER BY rf_idV006) AS 'N_SV', rf_idV006 AS 'USL_OK'
		...
		,(SELECT CASE WHEN age<18 THEN 0 WHEN age>17 AND age<60 THEN 1 ELSE 2 END AS 'VZST'
			...
			FROM  dbo.t_OrderAdult_104_2018 ss
			WHERE ss.ReportMonth=@reportMonth AND ss.rf_idV006=s.rf_idV006 and ISNULL(ss.AP_Type,9)=ISNULL(s.AP_Type,9)
...
FROM dbo.t_OrderAdult_104_2018 s
WHERE ReportMonth=@reportMonth
