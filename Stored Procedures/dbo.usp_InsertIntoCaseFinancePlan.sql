...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_InsertIntoCaseFinancePlan]
		@dtBegin DATETIME,	
		@reportYear SMALLINT
as
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=m.rf_idCase            
					INNER JOIN (SELECT MU,beginDate,endDate,unitCode,ChildUET,AdultUET FROM RegisterCases.dbo.vw_sprMU WHERE calculationType=1 
								UNION ALL 
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			r.id=c.rf_idRecordCasePatient															
					INNER JOIN dbo.t_MES m ON
			c.id=m.rf_idCase            
					INNER JOIN (SELECT MU,beginDate,endDate,unitCode,ChildUET,AdultUET FROM RegisterCases.dbo.vw_sprMU WHERE calculationType=2 
								UNION ALL 
