...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_IntersectionaStacionarAndDStac]
			@dtBeginReg DATETIME,	
			@dtEndReg DATETIME,
			@dtEndRAK DATETIME,		
			@codeSMO CHAR(5)
...
FROM AccountOMS.dbo.t_File f INNER JOIN AccountOMS.dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase  											
					INNER JOIN dbo.t_IntersectionStacAndDStac dp ON
...
FROM AccountOMS.dbo.t_File f INNER JOIN AccountOMS.dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase  											
					INNER JOIN dbo.t_IntersectionStacAndDStac dp ON
			c.id=dp.rf_idCaseDStac
			AND ps.ENP=dp.ENP
WHERE f.DateRegistration>=@dtBeginReg AND f.DateRegistration<=@dtEndReg AND a.rf_idSMO=@codeSMO AND a.ReportYear>=2017 AND c.rf_idV006=2  AND dp.DateRegistrationDStac>=@dtBeginReg AND dp.DateRegistrationDStac<=@dtEndReg 
...
drop TABLE #tmpCases
GO
GRANT EXECUTE ON  [dbo].[usp_IntersectionaStacionarAndDStac] TO [AccountsOMS]
GO
GRANT EXECUTE ON  [dbo].[usp_IntersectionaStacionarAndDStac] TO [db_AccountOMS]
GO
