...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_IntersectionaStacionarAndStac]
			@dtBeginReg DATETIME,	
			@dtEndReg DATETIME,
			@dtEndRAK DATETIME,		
			@codeSMO CHAR(5)
...
FROM AccountOMS.dbo.t_File f INNER JOIN AccountOMS.dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase  											
					INNER JOIN dbo.t_IntersectionStacAndStac dp ON
...
FROM AccountOMS.dbo.t_File f INNER JOIN AccountOMS.dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase  											
					INNER JOIN dbo.t_IntersectionStacAndStac dp ON
			c.id=dp.rf_idCaseStac2
			AND ps.ENP=dp.ENP
WHERE f.DateRegistration>=@dtBeginReg AND f.DateRegistration<=@dtEndReg AND a.rf_idSMO=@codeSMO AND a.ReportYear>=2017 AND c.rf_idV006=1  AND dp.DateRegistrationStac2>=@dtBeginReg AND dp.DateRegistrationStac2<=@dtEndReg 
...
FROM AccountOMS.dbo.t_PaymentAcceptedCase2 c INNER JOIN #tmpCases t ON
				c.rf_idCase=t.rf_idCase		
							LEFT JOIN vw_sprReasonDenialPayment_ForPDnReports r ON
...
drop TABLE #tmpCases
GO
GRANT EXECUTE ON  [dbo].[usp_IntersectionaStacionarAndStac] TO [AccountsOMS]
GO
GRANT EXECUTE ON  [dbo].[usp_IntersectionaStacionarAndStac] TO [db_AccountOMS]
GO
