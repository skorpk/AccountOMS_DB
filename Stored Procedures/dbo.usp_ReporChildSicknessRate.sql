...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_ReporChildSicknessRate]
as
...
--SELECT  DISTINCT p.rf_idV014, r.rf_idV009 
...
FROM (
	...
	UNION all
	-------Col2-----------
	SELECT 0,COUNT(DISTINCT PID) AS Col2, 0 AS Col3, 0 AS Col4, 0 AS Col5, 0 AS Col6, 0 AS Col7
	FROM #tPeople p INNER JOIN #tRSLT r ON
			p.rf_idV009=r.rf_idV009
	WHERE rf_idV014<3 AND AmountPayment>0 AND Letter IN('D','F','V','I','U')
	UNION ALL
	-------Col3-----------
	...
	-------Col6-----------
	UNION all
	SELECT 0,0 ,0,0,0,COUNT(DISTINCT PID) AS Col6,0
	FROM #tPeople p INNER JOIN dbo.vw_Diagnosis d ON
			p.rf_idCase=d.rf_idCase
	WHERE AmountPayment>0 AND Letter NOT IN('D','F','V','I','U') AND d.DS1 NOT LIKE 'Z%'
