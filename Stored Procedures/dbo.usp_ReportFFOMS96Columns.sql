...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_ReportFFOMS96Columns]
...
from #tCasesM WHERE AmountPay>0 AND DiffDay IN (0,1,2)	AND AttachLPU=CodeM

INSERT #tmpMagalyz( Col19)
SELECT COUNT(DISTINCT ENP) AS Col19
from #tCasesM WHERE AmountPay>0 AND DiffDay>2
											
INSERT #tmpMagalyz( Col20)
SELECT COUNT(DISTINCT ENP) AS Col20
from #tCasesM WHERE AmountPay>0 AND DiffDay>2	AND AttachLPU=CodeM
---------------------------------------------------
...
SELECT SUM(Col39) AS Col39,SUM(Col40) AS Col40,SUM(Col41) AS Col41,SUM(Col42) AS Col42,SUM(Col43) AS Col43,SUM(Col44) AS Col44
		,SUM(Col55) AS Col55,SUM(Col56) AS Col56,SUM(Col57) AS Col57,SUM(Col58) AS Col58,SUM(Col59) AS Col59,SUM(Col60) AS Col60
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=u.rf_idCase                  
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=@reportYear AND c.rf_idV006<4 AND a.rf_idSMO<>'34'	AND a.ReportMonth<=@reportMonth AND c.DateEnd<@dateEndCase
union all
--новое от 09.04.2019
SELECT DISTINCT c.id AS rf_idCase,r.id AS rf_idRecordCasePatient, c.AmountPayment,ps.ENP, c.AmountPayment AS AmountPay, u.rf_idN013
			,a.rf_idSMO AS CodeSMO
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=dd.rf_idCase 	
					left JOIN dbo.t_ONK_USL u ON
			c.id=u.rf_idCase				               
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=@reportYear AND c.rf_idV006<4 AND dd.DS_ONK=1 AND a.rf_idSMO<>'34' AND a.ReportMonth<=@reportMonth AND c.DateEnd<@dateEndCase
