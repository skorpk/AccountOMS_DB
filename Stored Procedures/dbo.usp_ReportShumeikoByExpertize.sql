...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[usp_ReportShumeikoByExpertize]		
			@dtBegin DATETIME,	
			@dtEndReg DATETIME,
			@dtEndRegAkt DATETIME,
			@v6 TINYINT,--меняем условия оказания мед.помощи
			@codeSMO CHAR(5)
as				
...
SELECT distinct c.rf_idCase,c.ENP,c.CodeM,l.NAMES,c.DateBegin,c.DateEnd,c.DS1,c.Account,c.DateAccount,c.idRecordCase,CAST(c.AmountPayment AS MONEY) AS AmountPayment,CAST(c.AmountPaymentAccepted AS money) AS AmountPaymentAccepted,
		c.DateAkt,CASE WHEN c.TypeCheckup=2 THEN 'МЭЭ' WHEN c.TypeCheckup=3 then 'ЭКМП' ELSE '' END AS Typechekup,CAST(ISNULL(c.AmountDeduction,0.0) AS MONEY) AS Deduction,c.reason,c.NewBorn
--INTO tmpStacionar
