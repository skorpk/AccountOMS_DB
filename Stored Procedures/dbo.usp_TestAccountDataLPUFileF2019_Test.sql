...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[usp_TestAccountDataLPUFileF2019_Test]
			@doc xml,
			@patient xml,
			--@file varbinary(max),
			@fileName varchar(26),
...
CREATE TABLE #tDS2_Info(GUID_CASE uniqueidentifier,DiagnosisCode VARCHAR(10) NOT NULL,	IsFirst TINYINT, IsNeedDisp tinyint)
CREATE TABLE #tPrescriptions
							(
...
begin
	exec dbo.usp_GetCaseFromRegisterCaseDBFilesF2019 @account,@codeMO,@month,@year
end		 
	...
	from #case t inner join #t5 t1 on
			...
			AND t.SL_ID=t1.SL_ID
			AND t.VBR=t1.VBR
			AND t.DATE_Z_1=t1.DATE_Z_1
			AND t.DATE_Z_2=t1.DATE_Z_2
			AND t.P_CEL=t1.P_CEL
			AND t.P_OTK=t1.P_OTK
			AND t.SUMV=t1.SUMV
			...
			IF EXISTS(SELECT * FROM #tPrescription b WHERE NOT EXISTS(SELECT * FROM #tPrescriptions WHERE GUID_Case=b.ID_C AND NAZR=b.NAZR
																		...
																		AND ISNULL(NAZ_USL,'bla-bla')=ISNULL(b.NAZ_USL,'bla-bla')
																		AND ISNULL(NAPR_DATE,'22220101')=ISNULL(b.NAPR_DATE,'22220101')
																		AND ISNULL(NAPR_MO,'blabla')=ISNULL(b.NAPR_MO,'blabla')
																		)
																		)
			BEGIN 			
				insert @et values(588,15)
			...
			IF EXISTS(SELECT * FROM #tDS2_N b WHERE NOT EXISTS(SELECT * FROM #tDS2_Info 
																WHERE GUID_Case=b.ID_C AND DiagnosisCode=b.DS2
																		AND ISNULL(IsFirst,9)=ISNULL(b.DS2_PR,9) 
																		AND IsNeedDisp=b.PR_D
																		))
			BEGIN 		
				--SELECT 2	
				insert @et values(588,15)
...
--внес изменения т.к при зачистке данных в базе Реестров сведений за Страмным удалил хирургические операции.
	...
	EXEC usp_GetMeduslugiFromRegisterCaseDB2019 @account
...
--------не верна работала выборка файла. не был учтен отчетный год			
	...
	from(
			...
			AND ISNULL(t.DOCDATE,'22220101')=ISNULL(t1.DOCDATE,'22220101')
			AND ISNULL(t.DOCORG,'bla-bla')=ISNULL(t1.DOCORG,'bla-bla')
			WHERE t1.ID_PAC='9A7F1051-3DEC-41A7-98C0-6AF7ACBC00E3'
...
select distinct r1.ID_Patient,rp.Fam,rp.Im,rp.Ot,rp.rf_idV005 as W,rp.BirthDay as DR,ra.Fam as Fam_P, ra.Im as IM_P, ra.Ot as Ot_P,ra.rf_idV005 as W_P,
			...
			from RegisterCases.dbo.t_FileBack f inner join RegisterCases.dbo.t_RegisterCaseBack a on 
				...
				rp.id=ra.rf_idRegisterPatient
									left join RegisterCases.dbo.t_RegisterPatientDocument doc on
				rp.id=doc.rf_idRegisterPatient
WHERE r1.ID_Patient='9A7F1051-3DEC-41A7-98C0-6AF7ACBC00E3'
