...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[usp_TestAccountDataLPUFileH_Test]
			@doc xml,
			@patient xml,
			--@file varbinary(max),
			@fileName varchar(26),
...
begin
	...
	from #case t inner join #t5 t1 on
			...
			BEGIN 		
			SELECT * FROM #ONK_SL_RC t INNER JOIN #ONK_SL tt ON tt.ID_C=t.GUID_CASE WHERE t.DS1_T IS NOT NULL AND  NOT EXISTS(SELECT * 
																						  FROM #ONK_SL k 
																						  WHERE k.ID_C=t.GUID_CASE AND k.DS1_T=t.DS1_T AND 
																						  t.STAD=k.STAd and k.ONK_T=t.ONK_T AND k.ONK_N=t.ONK_N
																						  AND k.ONK_M=t.ONK_M AND ISNULL(k.MTSTZ,0)=ISNULL(t.MTSTZ,0) 
