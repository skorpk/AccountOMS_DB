...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[usp_TestAccountDataLPUFilesFileF]
			@doc xml,
			@patient xml,
			--@file varbinary(max),
			@fileName varchar(26),
...
insert #t5( N_ZAP, ID_PAC ,IDCASE ,ID_C ,DISP ,USL_OK ,VIDPOM ,FOR_POM ,LPU ,PROFIL ,VBR ,NHISTORY ,P_OLK ,DATE_1 ,DATE_2 ,DS1 ,
...
--new table
CREATE TABLE #tDispInfo(GUID_CASE uniqueidentifier,TypeDisp VARCHAR(3),IsMobileTeam BIT,TypeFailure TINYINT)

CREATE TABLE #tDS2_Info(GUID_CASE uniqueidentifier,DiagnosisCode VARCHAR(10) NOT NULL,	IsFirst BIT, IsNeedDisp tinyint)
CREATE TABLE #tPrescriptions(GUID_CASE uniqueidentifier,NAZR TINYINT ,rf_idV015 SMALLINT,TypeExamination TINYINT,rf_dV002 SMALLINT,rf_idV020 SMALLINT)
...
begin
	...
	from #case t inner join #t5 t1 on
			...
			IF EXISTS(SELECT * FROM #tPrescription b WHERE NOT EXISTS(SELECT * FROM #tPrescriptions WHERE GUID_Case=b.ID_C AND NAZR=b.NAZR 
																		AND rf_idV015=b.NAZ_SP AND TypeExamination=b.NAZ_V AND rf_dV002=bNAZ_PMP AND rf_idV020=b.NAZ_PK  ))
			BEGIN 			
				insert @et values(588,15)
			END
			
			IF EXISTS(SELECT * FROM #tDS2_N b WHERE NOT EXISTS(SELECT * FROM #tDS2_Info WHERE GUID_Case=b.ID_C AND TypeDisp=b.DISP
																		AND IsFirst=b.DS_PR AND IsNeedDisp=b.PR_D ))
			BEGIN 			
				insert @et values(588,15)
			END
			IF EXISTS(SELECT * FROM #t5 b WHERE NOT EXISTS(SELECT * FROM #tDispInfo WHERE GUID_Case=b.ID_C AND DiagnosisCode=b.DS2 
																		AND IsMobileTeam=b.VBR AND TypeFailure=b.P_OTK ))
			BEGIN 			
				insert @et values(588,15)
			END
...
--внес изменения т.к при зачистке данных в базе Реестров сведений за Страмным удалил хирургические операции.
	...
	CREATE TABLE #meduslugi 
		(
			...
			IsChildTariff bit NOT NULL,	  --пусто
			DateHelpBegin date NOT NULL,
			DateHelpEnd date NOT NULL,
			DiagnosisCode char(10) NOT NULL, --пусто
...
GO
GRANT EXECUTE ON  [dbo].[usp_TestAccountDataLPUFilesFileF] TO [db_AccountOMS]
GO
