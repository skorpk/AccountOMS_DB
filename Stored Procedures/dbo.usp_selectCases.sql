...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_selectCases]
--declare
...
--                c.DateEnd AS Окончен,c.AmountPayment AS Выставлено,v9.Name AS Результат,v12.Name AS Исход,v4.Name AS СпециальностьМедРаботника,v10.Name AS СпособОплаты,rp.Fam + ' ' + rp.Im + ' ' + ISNULL(rp.Ot, '') AS Пациент,
...
--					,rp.Fam + '' '' + rp.Im + '' '' + ISNULL(rp.Ot,''''),rp.BirthDay,c.age,rp.BirthPlace,rcp.SeriaPolis,rcp.NumberPolis,f.DateRegistration,f.CodeM ,d.DS1,ra.Account,ra.[DateRegister]
...
GO
GRANT EXECUTE ON  [dbo].[usp_selectCases] TO [AccountsOMS]
GO
GRANT EXECUTE ON  [dbo].[usp_selectCases] TO [db_AccountOMS]
GO
